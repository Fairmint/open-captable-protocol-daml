...
[`scripts/test-imports.ts`](./scripts/test-imports.ts) for the executable package-surface contract.
`resolveOpenCapTableDarPath` checks `OPEN_CAP_TABLE_DAR_PATH`, then the packaged DAR, then optional
local-checkout paths; see its
[`source and options`](./scripts/npm-published-lib/openCapTableDarPath.ts) for exact resolution
behavior.
...
Read [Contract architecture](./docs/architecture.md) for the lifecycle, generated aggregate, and
links to the implementing DAML modules.
...
[`daml.yaml`](./OpenCapTable-v34/daml.yaml), [`multi-package.yaml`](./multi-package.yaml),
[`package.json`](./package.json), and `scripts/` remain the source of truth for current versions,
...
See [Development, testing, and releases](./docs/development-and-releases.md) for the generated-file
boundary and specialized checks.
