...
`UpdateCapTable` processes deletes and edits in caller-supplied order, then sorts creates by their
configured dependency tier. Creates can therefore introduce base objects before issuances that
reference them. Callers must order interdependent deletes and edits themselves; each edit archives
and recreates the affected object inside the same transaction. The aggregate itself is recreated
with the new maps, so callers must use the returned `CapTable` contract ID.
...
source of truth for create tiers and most generated cross-object reference checks; aggregate-wide
checks such as Financing issuance references live in the templates.
