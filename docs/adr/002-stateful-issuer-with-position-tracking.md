...
**Implemented** | 2026-01-05 | **Updated** 2026-01-08 (Batch API)
...
## Batch API (2026-01-08 Update)

### Motivation

The original design used individual choices per operation (CreateStakeholder, EditStakeholder, DeleteStakeholder, etc.), resulting in ~140 choices. Each operation created a new CapTable contract, which was inefficient for bulk operations.

For example, importing 100 stakeholders required 100 separate transactions, each creating a new CapTable.

### Batch Design

A single `UpdateCapTable` choice replaces all individual create/edit/delete choices:
...
-- Edits: wrapper records with id + data
data EditStakeholderData = EditStakeholderData with
    edit_stakeholder_id: Text
    edit_stakeholder_data: StakeholderOcfData

data OcfEditData
  = OcfEditStakeholder EditStakeholderData
  | OcfEditStockClass EditStockClassData
  -- ... all types
...
- Type discovery: Scans `OpenCapTable-v25/daml/Fairmint/OpenCapTable/OCF/` for DAML files
