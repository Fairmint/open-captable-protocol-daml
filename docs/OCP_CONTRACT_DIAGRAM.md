...
```
OpenCapTable-v28/daml/Fairmint/OpenCapTable/
...
**Code Generation**: `CapTable.daml` is generated by `scripts/codegen/generate-captable.ts`, which discovers OCF types from `OpenCapTable-v28/daml/Fairmint/OpenCapTable/OCF/` and generates the appropriate Maps and CRUD choices. Run `npm run codegen:captable` to regenerate.
