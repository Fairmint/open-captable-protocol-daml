...
- **RewardShareConfig**: Configuration for a single recipient (percentage, recipient, provider)
...
- Recipients are automatically set as observers
...
1. Call `RewardShare_Distribute` (nonconsuming) with:
   ...
   - Distribution round number (must be > `lastRoundProcessed` and <= current OpenMiningRound)
   - Expected DSO party
...
2. The choice:
   - Validates round progression (distribution round must be <= current OpenMiningRound)
...
Each distribution updates the reward share's `lastRoundProcessed` and validates:
