...
function findPackageFolder(packageName: string): string {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      // Match patterns like "Subscriptions-v09", "OpenCapTable-v25", etc.
...
function performMajorUpgrade(info: PackageInfo): void {
  ...
  // Replace full version strings (e.g., "Subscriptions-v09-0.2.3" → "Subscriptions-v09-0.0.4")
  ...
  // Replace major version strings (e.g., "Subscriptions-v09" → "Subscriptions-v09")
