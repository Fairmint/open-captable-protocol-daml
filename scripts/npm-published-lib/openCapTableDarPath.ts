...
function resolveCustomSiblingDar(
  options: ResolveOpenCapTableDarPathOptions,
): string | null {
 ...
 */
export function resolveOpenCapTableDarPath(
  options?: ResolveOpenCapTableDarPathOptions,
): string {
  const opts = options ?? {};
  ...
  if (
    customRaw &&
    !path.isAbsolute(customRaw) &&
    !opts.siblingSearchFrom?.trim()
  ) {
    throw new Error(
  ...
  try {
    ...
    const base =
      packagedCause instanceof Error
        ? packagedCause.message
        : String(packagedCause);
    const hints: string[] = [];
    ...
    if (from) {
      hints.push(
        "No DAR at ../open-captable-protocol-daml/published-dars/OpenCapTable.dar.",
      );
    } else {
      hints.push(
